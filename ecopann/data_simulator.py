...
#%%
#test, to be removed?
class AddGaussianNoise_multiBranch(AddGaussianNoise):
    """Add Gaussian noise to simulated data for the multibranch network.
    ...
    def __init__(self, spectra, params=None, obs_errors=None, cholesky_factor=None, 
        ...
        if use_GPU:
            ...
            self.epsilon = torch.FloatTensor([1e-20])
        #new
        self.get_spectra_length()
        # self.use_err = 
        # self.use_cov = self._use_cov(self.cholesky_factor)
    
    def get_spectra_length(self):
        if self.is_list:
            self.spectra_length = [self.spectra[i].size(1) for i in range(len(self.spectra))]
            self.spectra_length_max = max(self.spectra_length)
        else:
            self.spectra_length = self.spectra.size(1)
            self.spectra_length_max = self.spectra_length
    
    # def _use_error(self, obs_errors):
    #     use_err = False
        
    # def _use_cov(self, cholesky_factor):
    #     use_cov = False
    #     if self.is_list:
    #         for i in range(len(self.spectra)):
    #             if cholesky_factor[i] is not None:
    #                 use_cov = True
    #     else:
    ...
    def obs_noise_uncorr(self, spectrum, obs_error=None):
        if self.use_GPU:
            noise = torch.cuda.FloatTensor(spectrum.size()).normal_(0,1) * obs_error
        else:
            noise = torch.FloatTensor(spectrum.size()).normal_(0,1) * obs_error
    ...
    def obs_noise_corr(self, spectrum, cholesky_factor=None):
        if self.use_GPU:
            mean = torch.zeros(cholesky_factor.size(-1)).cuda()
        else:
            mean = torch.zeros(cholesky_factor.size(-1))
    ...
    #remove factor_sigma?
    def multiNormalSpectra(self, factor_sigma=0.5):
        #to be updated by using use_cov & use_err? update for is_list is False?
        if self.use_GPU:
            # method 1
            # error_factor_1 = torch.abs(torch.cuda.FloatTensor(self.spectra_length_max).normal_(0, factor_sigma)) #A !!!+, use this
            # #try to update this to make the result better???
            # #the epsilon is used to ensure error_factor>0, because the diagonal of cholesky_factor*error_factor must be positive
            # # error_factor_2 = torch.abs(torch.cuda.FloatTensor(1).normal_(0, factor_sigma)) + self.epsilon #A !!, use this
        ...
        for i in range(len(self.spectra)):
            if self.cholesky_factor[i] is None:
                #method 1
                # noise = self.obs_noise_uncorr(self.spectra[i], obs_error=self.obs_errors[i]*error_factor_1[:self.spectra_length[i]])
                
                #method 2
                # noise = self.obs_noise_uncorr(self.spectra[i], obs_error=self.obs_errors[i]*error_factor)
                
                 #method 3
                noise = self.obs_noise_uncorr(self.spectra[i], obs_error=self.obs_errors[i]*error_factor_1[:self.spectra_length[i]])
            else:
                # method 1
                # # noise = self.obs_noise_corr(self.spectra[i], cholesky_factor=self.cholesky_factor[i]*error_factor_2)
                # noise = self.obs_noise_corr(self.spectra[i], cholesky_factor=self.cholesky_factor[i]*error_factor_1[0])
                
                #method 2
                # noise = self.obs_noise_corr(self.spectra[i], cholesky_factor=self.cholesky_factor[i]*error_factor)
                
                # method 3
                noise = self.obs_noise_corr(self.spectra[i], cholesky_factor=self.cholesky_factor[i]*error_factor_2)
                
                
            _noisy_sp = self.spectra[i] + noise
            noisy_sp.append(_noisy_sp)
        return noisy_sp
